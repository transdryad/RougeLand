...
void GameMap::drawInBounds(const int x, const int y, const int nx, const int ny, TCODBsp* node) {
    ...
    for (int i = 0; i < inum; i++) {
        ...
        if (isWalkable(ix, iy)) {
            ...
            switch (itype) {
                case SWORD:
                    items.emplace_back(itype, ix, iy, random->getInt(1, 4), game, game.level); break;
                case HELMET:
...
void GameMap::compute() {
    ...
    if (game.level != 0) {
        tiles[rooms.front().cx][rooms.front().cy] = up_stair; //up staircase
    }
    tiles[rooms.back().cx][rooms.back().cy] = down_stair; //down staircase
    for (int y = 0; y < 45; y++) {
        for (int x = 0; x < 80; x++) {
